...
The reference is a name or an id, resolved exactly the way `smith resume` and `smith sessions delete` resolve one. The Markdown carries the session name, the provider and model, the timestamps, the token counts, the cost and the todo list, then every message in order — roles, text, thinking, tool calls and their results:
...
Four things the format is careful about:
...
- **Tool arguments, tool results and thinking are abbreviated** to 800 characters, and the fences grow past any backticks inside them so a `bash` result full of Markdown cannot break the document. `--json` is the lossless form: one JSON object with the metadata and every message, unabbreviated.
...
Damage degrades instead of aborting: a damaged session index costs the export nothing but a name lookup, an unreadable line in the transcript is skipped, a session file that is gone or will not parse falls back to the raw transcript, and a directory that has lost everything says so and exits 1. Nothing on this path builds a provider, needs an API key or touches the network.
