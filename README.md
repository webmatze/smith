...
Nothing an MCP probe starts outlives it, however badly it behaves: a server that ignores `SIGTERM` is killed outright, and the shutdown runs after the deadline rather than inside it. Nothing the report prints comes from a server either. A stdio server inherits smith's environment, and an HTTP server chooses what goes in its error body — a gateway echoing an `Authorization` header back is not hypothetical — so a server's own words stay out of a page meant to be pasted into a bug report, and only what smith composed itself (a status code, a timeout) goes in. Neither does anything out of `mcp.json` beyond what identifies a server: an argument list is summarised by its length and a url is cut back to scheme, host and port, because `--api-key` and `https://token@host/` are both ordinary ways to configure a server.
